...
EOF;
	...
	protected function simplify( $data ) {
		return ( new WPCOM_JSON_API_Menus_Simplifier( $data ) )->translate();
	}

	protected function complexify( $data ) {
		return ( new WPCOM_JSON_API_Menus_Complexify( $data ) )->translate();
	}
}
	...
	protected function add_widget_locations( $menus ) {
		...
		foreach ( $menus as &$menu ) {
			...
			foreach ( $nav_menu_widgets as $key => $widget ) {
				if ( is_array( $widget ) && $widget['nav_menu'] == $menu['id'] ) {
